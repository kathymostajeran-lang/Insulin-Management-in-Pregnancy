...
- Mobile-first shell: a **sticky top bar** with a `<select>` switcher (grouped by `<optgroup>`: everyday / around-delivery / special) + compact TDD, a **collapsible patient-inputs panel** (`<details>`, with a live one-line summary), then the active view. Wires inputs → `model.ts`/`dosing.ts` → the active module. Contains no clinical rules itself.
- **Guided landing (`HomeMenu`).** The default view is a plain-language "What do you need to do?" menu of task cards (each module carries a `title` + "use this when…" `when` line and a `group`), so a clinician who doesn't already know the tool is routed to the right module. `go(view)` centralizes navigation: it opens the inputs panel inside a module and collapses it on the menu. Every module shows a "‹ All tasks" back link. This exists to answer real usability feedback that the app was "complex to the point that if you know how to use it, you don't need it."
