...
- Mobile-first shell: inside a module, a **sticky top bar** with a `<select>` switcher (grouped by `<optgroup>`: everyday / urgent / around-delivery / setup&monitoring, option text = each module's friendly `title`) + compact TDD; the switcher is hidden on the home menu. Wires inputs → `model.ts`/`dosing.ts` → the active module. Contains no clinical rules itself.
- **Guided landing (`HomeMenu`).** The default view is a plain-language "What do you need to do?" menu of task cards (each module carries a `title` + "use this when…" `when` line and a `group`), so a clinician who doesn't already know the tool is routed to the right module. `go(view)` centralizes navigation; every module shows a "‹ All tasks" back link.
- **Scoped inputs (`needsInputs`/`showsTdd`).** The shared patient-inputs `<details>` panel renders **only** for modules that read it (`needsInputs`: start/adjust/pump/postpartum) and the topbar/banner TDD **only** for modules that read `model.tdd` (`showsTdd`: start/adjust). Self-contained modules (labor/DKA/steroids/hypo/CGM) get no shared form or inert TDD.
- **Demo is opt-in and badged.** The app boots on `EMPTY_INPUTS`, never a pre-filled patient. "Demo prefill" sets `isDemo`, which renders an unmistakable "Demonstration data — not your patient" badge (topbar + inputs summary); any real edit or Clear drops the flag. This is a safety mitigation — a demo dose must never read as a live recommendation.
...
- **Home triage.** An optional "Not sure? Answer 2 questions" panel on `HomeMenu` (already-on-insulin? · pregnant/labor/delivered?) routes to Start / Adjust / Labor / Postpartum via `onPick`. The nine cards remain for users who know where they're going.
- **Print one-pager.** `printSummary()` expands every `<details>` (so math/sources print), calls `window.print()`, and restores collapsed state on `afterprint`. The `@media print` block in `app.css` hides interactive chrome (topbar, buttons, advanced knobs) and keeps the title, patient context, the module output, the demo badge, and the safety footer — a chart-ready summary.
