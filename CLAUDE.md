...
│   ├── AidTab.tsx       # AID/HCL: recommendation grades, target-vs-pregnancy flag, effect sizes (§8)
...
Per the spec's suggested order (§17), implemented so far: data/target service, the TDD/initiation calculator with the C-02 switch, pattern titration, the fixed/calculated correction, CSII (C-14), the intrapartum table (C-15 gap surfaced), the CGM adapter (§6: scorecard, HS-09/10/11 guards, tagged-value derivation feeding the titration engine, basal-hyperglycemia signal, phenotype triggers), Postpartum (§13: all dose methods shown side by side per C-13, target tiers, lactation guidance), Hypoglycemia (§12: C-01 threshold check, outpatient Rule of 15, the UC23 inpatient rescue ladder routed by consciousness/PO/BG, glucagon, symptoms, unawareness), Steroids (§9: **Mathiesen ER** betamethasone day-by-day insulin adjustment with taper-back safety), DKA (§10: the fluid/electrolyte protocol as reference plus the **Yale IV insulin infusion** drip calculator — user-requested; UC23 orders otherwise not automated, per HS-14), and AID (§8: recommendation grades, the F.2 target-vs-pregnancy-range flag, gated assistive techniques, ES25/AiDAPT effect sizes, C-18 peripartum note).

**Every module in the spec's build order (§17) is now implemented — 11 tabs.** A few engine functions are tested in `dosing.ts` but not currently surfaced in the UI (`dkaDiagnosis`, `dkaIcuCriteria`, `steroidEpisode`, `steroidBgAction`); resurface them if needed. See spec §18 for clinical gaps the four sources do not cover (twins, bariatric, CKD, MODY, device-specific AID targets, etc.) — future work needs additional references.
